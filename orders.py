...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi('orderscreen.ui',self)
        self.show()
        self.populate_table()
        self.searchOrders.clicked.connect(self.search_order)
        self.deleteOrder.clicked.connect(self.delete_order)
        self.exitButton.clicked.connect(self.closeWindow)
        self.orderList.setColumnWidth(1,90)

        self.open_update_window = None
        self.modifyOrder.clicked.connect(self.openUpdateOrder)

        self.addOrder.clicked.connect(self.openInsertOrder)
        self.open_insert_window = None
    ...
    def openReport(self):
        self.open_report_window = open_report()
    ...
    def openUpdateOrder(self):
        ...
        if selected_row >= 0:
            ...
            if self.open_update_window is None:
                self.open_update_window = update_order(row_data)
    ...
    def openInsertOrder(self):
        self.open_insert_window = insert_order()
        self.open_insert_window.show() 
    ...
    def populate_table(self):
        ...
        selectOrderQuery = "SELECT * FROM orders"
    ...
    def search_order(self):
        ...
        if search_text.strip() == '':  # Check if search text is empty
            # Fetch all records from the orders table
            search_query = "SELECT * FROM orders"
            cursor.execute(search_query)
        else:
            if search_criteria == 'CustomerID':
                search_query = "SELECT * FROM orders WHERE [customerID] = ?"
                cursor.execute(search_query, (search_text,))
            elif search_criteria == 'Order Number':
                search_query = "SELECT * FROM orders WHERE [orderNumber] = ?"
                cursor.execute(search_query, (search_text,))
    ...
    def delete_order(self):
        ...
        if selected_row >= 0:  
            ...
            if confirmation == QtWidgets.QMessageBox.StandardButton.Yes:
                ...
                delete_associated_query = "DELETE FROM itemOrder WHERE orderNumber = ?"
                cursor.execute(delete_associated_query, (order_id,))
                connection.commit()

                delete_query = "DELETE FROM orders WHERE orderNumber = ?"  
...
class view_order(QtWidgets.QMainWindow):
    ...
    def populate_viewOrderTable(self, row_data):
        order_id = row_data[0]  # Assuming orderNumber is the first item in row_data
        customer_id = row_data[1]  # Assuming customerID is the second item in row_data

        connection_string = f'DRIVER={{ODBC Driver 17 for SQL Server}};SERVER={server};DATABASE={database};Trusted_Connection=yes;'
        connection = pyodbc.connect(connection_string)
        cursor = connection.cursor()

        # Set the text of the QLineEdit with the CustomerID
        self.viewCustomerIDLine.setText(str(customer_id))
        self.viewCustomerIDLine.setEnabled(False)  # Disable the line edit

        # Clear previous table contents
        self.viewOrderTable.clearContents()
        self.viewOrderTable.setRowCount(0)

        selectOrderQuery = "SELECT * FROM itemOrder WHERE orderNumber = ?"
...
class open_report(QtWidgets.QMainWindow):
    def __init__(self):
        super(open_report,self).__init__()
        uic.loadUi("salesReport.ui",self)
        self.show()
        self.generateButton.clicked.connect(self.populate_table)
        self.exitButton.clicked.connect(self.closeWindow)
    ...
    def populate_table(self):
        ...
        order_data = """
        select count(orderNumber),sum(orderAmount) from orders 
        where year(orderDate) = ? and month(orderDate) = ?
        """
        cursor.execute(order_data,year,month)
        result = cursor.fetchone()

        po_data = """
        select count(purchaseOrderNumber),sum(amountPaid) from purchaseOrder 
        where year(dateOfPurchase) = ? and month(dateOfPurchase) = ?
        """
        cursor.execute(po_data,year,month)
        result2 = cursor.fetchone()
        profit = result[1] - result2[1]

        self.ordersEdit.setText(str(result[0]))
        self.ordersEdit.setReadOnly(True)
        self.revenueEdit.setText(str(result[1]))
        self.revenueEdit.setReadOnly(True)
        self.poEdit.setText(str(result2[0]))
        self.poEdit.setReadOnly(True)
        self.expensesEdit.setText(str(result2[1]))
        self.expensesEdit.setReadOnly(True)
        self.netProfitEdit.setText(str(profit))
        self.netProfitEdit.setReadOnly(True)
        connection.commit()
        connection.close()

class insert_order(QtWidgets.QMainWindow):
    def __init__(self):
        super(insert_order,self).__init__()
        uic.loadUi("finalAddOrders.ui",self)
        self.show()
        ...
        self.itemDetailsTable.setColumnWidth(0,90)
        self.order_num = 0

        self.customerDetailsTable.setColumnWidth(0,70)
    ...
    def populate_customers_table(self):
        ...
        selectCustomersQuery = "select * from customerDetails"
        cursor.execute(selectCustomersQuery)
        for row_index, row_data in enumerate(cursor.fetchall()):
            self.customerDetailsTable.insertRow(row_index)
            for col_index, cell_data in enumerate(row_data):
                item = QTableWidgetItem(str(cell_data))
                self.customerDetailsTable.setItem(row_index, col_index, item)
    ...
    def search_customer(self):
        ...
        self.customerDetailsTable.clearContents()
        self.customerDetailsTable.setRowCount(0)

        if search_text.strip() == '':  
            search_query = "SELECT * FROM customerDetails"
            cursor.execute(search_query)
        else:
    ...
    def add_customer(self):
        ...
        QtWidgets.QMessageBox.information(self, 'Success', 'Customer has been added', QtWidgets.QMessageBox.StandardButton.Ok)
    ...
    def select_customer(self):
        ...
        selected_row = self.customerDetailsTable.currentRow()
        if selected_row >= 0:
            customer_id = self.customerDetailsTable.item(selected_row, 0).text()
            if self.orderInserted==False:
                ...
                create_order_query = """
                        ...
                        values (?,?,?,?)
                        """
                cursor.execute(create_order_query, (customer_id, status, selected_date, amount))
                connection.commit() 
    ...
    def add_item(self,row_data):
        ...
        connection = pyodbc.connect(connection_string)
        cursor = connection.cursor()
        if self.customerSelected==False:
            QtWidgets.QMessageBox.warning(self, 'Warning', 'Please select a customer before generating order.', QtWidgets.QMessageBox.StandardButton.Ok)
            return 
        #reading the inputs from the line edits
        ...
        add_items_query = """
                    insert into itemOrder ([orderNumber],[itemID],[itemName],[itemQtyBought],[itemColour],[pricePerUnit])
                    values (?,?,?,?,?,?)
                    """
        cursor.execute(add_items_query,(orderNum,itemID,itemName,quantity,colour,float(price))) 


        #populating the itemOrder list
        populate_item_query = """
                select itemID,itemName,itemQtyBought,itemColour,pricePerUnit from itemOrder
                where orderNumber = ?
                            """
        ...
        if (int(qty_avail)>int(quantity)):
            update_qty = int(qty_avail) - int(quantity)
            cursor.execute(increase_items,update_qty,itemID)
        else:
            QtWidgets.QMessageBox.warning(self, 'Warning', 'There are only ' + str(qty_avail) + ' items of ' + itemID + ' available. Kindly reduce quantity or choose another product', QtWidgets.QMessageBox.StandardButton.Ok)
            return
    ...
    def order_done(self,event):
        ...
        # SQL query to update order amount using a subquery
        update_query = """
        UPDATE orders
        SET orderAmount = (
            select sum(pricePerUnit*itemQtyBought)
            from itemOrder
            where itemOrder.orderNumber = orders.orderNumber
            )
        """
        ...
        connection.close()
        if self.orderInserted==True and self.customerSelected==True:
            msg = QtWidgets.QMessageBox()
            msg.setIcon(QtWidgets.QMessageBox.Icon.Information)
            msg.setText("Order has been added.")
            msg.setWindowTitle("Success")
            msg.setStandardButtons(QtWidgets.QMessageBox.StandardButton.Ok)
            msg.exec()
            if msg.clickedButton() == msg.button(QtWidgets.QMessageBox.StandardButton.Ok):
                repopulate = MainWindow()
                repopulate.populate_table()
                self.close()  
        elif self.customerSelected==True and self.orderInserted==False:
            QtWidgets.QMessageBox.warning(None,"Empty Order","You are entering an empty order. You can modify it later.",QtWidgets.QMessageBox.StandardButton.Ok)
            self.close() 
        else: 
            QtWidgets.QMessageBox.warning(None,"No Order","You are exiting without adding an order",QtWidgets.QMessageBox.StandardButton.Ok)
            self.close() 

class update_order(QtWidgets.QMainWindow):
    def __init__(self,row_data):
        super(update_order,self).__init__()
        uic.loadUi("modifyOrder.ui",self)
        self.show()
        ...
        self.orderDetails.setColumnWidth(0,90)
        self.order_number = 0
    ...
    def update_table(self):
        ...
        connection = pyodbc.connect(connection_string)
        cursor = connection.cursor()
        # Iterate through modified_data dictionary and update the database with the modified values
        new_val = (self.orderDetails.item(self.row, self.col).text())

        item_id = self.orderDetails.item(self.row, 0).text()  
        update_query = "UPDATE itemOrder SET [itemQtyBought] = ? WHERE [itemID] = ?"
        update_amount = """UPDATE orders
            SET orderAmount = (
            select sum(pricePerUnit*itemQtyBought)
            from itemOrder
            where itemOrder.orderNumber = orders.orderNumber
        ...
        selectOrderQuery = "SELECT itemID, itemName, itemQtyBought, itemColour, pricePerUnit FROM itemOrder WHERE orderNumber = ?"
    ...
    def remove_item(self):
        ...
        if selected_row >= 0:  
            ...
            if confirmation == QtWidgets.QMessageBox.StandardButton.Yes:
                ...
                delete_associated_query = "DELETE FROM itemOrder WHERE itemID = ?"
    ...
    def change_done(self):
        ...
        cursor.execute(update_status,status,self.order_number)
        connection.commit() 
